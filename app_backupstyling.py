...
def retrieve_options(data_type=None):
    ...
    list_dirs = ["ssh", "-o", "StrictHostKeyChecking=no", "***REMOVED***", "python", "***REMOVED***/list_subdirs.py", "{chosen_dir}".format(chosen_dir=chosen_dir)]
...
def update_output(email_address, data_type_dir, model_name, contents_test, name_test, date_test, 
    ...
    if contents_test is not None and contents_adapt is not None:
        ...
        scp = """ssh -oStrictHostKeyChecking=no ***REMOVED*** mkdir -p {session_dir} && 
        scp -oStrictHostKeyChecking=no {test} {adapt} ***REMOVED***:{session_dir}""".format(session_dir = session_dir, test=test_path, adapt=adapt_path)
        subprocess.call(scp, shell=True)
        algorithm = model_name.split("_")[0]
        execute = 'ssh -oStrictHostKeyChecking=no ***REMOVED*** "bash -s" < execute_remote.sh {project_dir} {model_name} {data_type_dir} {session_id} {algorithm} {email_address}'.format(project_dir = project_dir, model_name=model_name, data_type_dir = data_type_dir, session_id=session_id, algorithm=algorithm, email_address = email_address) 
