...
def update_output(email_address, data_type_dir, model_name, contents_test, name_test, date_test, 
    ...
    if contents_test is not None and contents_adapt is not None:
        ...
        testscp = 'ssh -o "StrictHostKeyChecking=no" piebar@mentat004.dccn.nl mkdir -p {session_dir} && scp -o "StrictHostKeyChecking=no" {test} {adapt} piebar@mentat004.dccn.nl:/project_cephfs/3022051.01/sessions/f218a52645724b1fa7400e3c4248d876'.format(session_dir = session_dir, test=test_path, adapt=adapt_path)
        subprocess.call(scp, shell=True)
        algorithm = model_name.split("_")[0]
        # os path join does something strange with attaching two paths
        bash_path = os.path.join(projectdir, scriptdir, executefile).replace("\\","/") #"/project_cephfs/3022051.01/test_scripts/server/execute_modelling.sh"#
        print(f'{scp=},{testscp=}')
        ...
        testexecute = 'ssh -o "StrictHostKeyChecking=no" piebar@mentat004.dccn.nl /project_cephfs/3022051.01/test_scripts/server/execute_modelling.sh "/project_cephfs/3022051.01" "Somemodel" "ThickAvg" "f218a52645724b1fa7400e3c4248d876" "Somemodel" "pieter.barkema@donders.ru.nl"'
